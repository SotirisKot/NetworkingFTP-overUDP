...
//Ξεκινάει η εκτέλεση του thread
  ...
  private void transferFile(byte[] buffer, InetAddress address, int portNumber) throws IOException {
      ...
      //Θέτουμε το load να είναι maxPayload - 8 δίοτι έχω συνολικά 8 bytes ένα για το header και τα υπόλοιπα για το data.
      ...
      //Θέτουμε timeout 1500 όπως έχει διευκρινιστεί.
      int timeout = 1500;
      ...
      while (!file_sent){
          ...
          if(state.equals("packet_send")){
              ...
              progress = Math.floor((((double)PacketCounter*load) / buffer.length)*100);
              ...
              try{
                  ...
                  if(p.getAckPacket() && p.getAckNum() == sequence_num){
                      ...
                      System.out.println("\n" + "Received ack for last packet!!!");
                      System.out.println("\n" +"Total packets sent: " + PacketCounter);
                      file_sent = true;
                  }else{
                      System.out.println("\n" +"Duplicate ack..Ignoring it!!!");
                      DuplicateCounter++;
                      state = "wait_ack";
                  }
              }catch (IOException e){
                  System.out.println("\n" +"Timed out...must send packet again!!!");
